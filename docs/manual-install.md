...
```bash
sudo install -d /usr/src/x120x-0.4.8/src
sudo cp dkms.conf Makefile LICENSE /usr/src/x120x-0.4.8/
sudo cp src/x120x.c src/Kbuild /usr/src/x120x-0.4.8/src/
```

#### Step 3 — Build and install the kernel module

```bash
sudo dkms add x120x/0.4.8
sudo dkms build x120x/0.4.8
sudo dkms install x120x/0.4.8
```
...
You should see `x120x/0.4.8, <kernel-version>, aarch64: installed`.
