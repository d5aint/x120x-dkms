...
```bash
sudo install -d /usr/src/x120x-0.4.10/src
sudo cp dkms.conf Makefile LICENSE /usr/src/x120x-0.4.10/
sudo cp src/x120x.c src/Kbuild /usr/src/x120x-0.4.10/src/
```

#### Step 3 — Build and install the kernel module

```bash
sudo dkms add x120x/0.4.10
sudo dkms build x120x/0.4.10
sudo dkms install x120x/0.4.10
```
...
You should see `x120x/0.4.10, <kernel-version>, aarch64: installed`.
