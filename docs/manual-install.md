...
```bash
sudo install -d /usr/src/x120x-0.4.11/src
sudo cp dkms.conf Makefile LICENSE /usr/src/x120x-0.4.11/
sudo cp src/x120x.c src/Kbuild /usr/src/x120x-0.4.11/src/
```

#### Step 3 — Build and install the kernel module

```bash
sudo dkms add x120x/0.4.11
sudo dkms build x120x/0.4.11
sudo dkms install x120x/0.4.11
```
...
You should see `x120x/0.4.11, <kernel-version>, aarch64: installed`.
